<?php
	Class Lupo extends Mob{
		...
		public function subisciDanno(Danno $da){
			$msg = '';
			if(rand(0, 10) > 7){
				$msg .= $this->getNome() . ' effettua una schivata e contrattacca!' . "\n";
		...
		public function s_0(){
			$msg = '';
			//Morso
			if($this->getDistanceFrom($this->target) > 1) return false;
			$msg .= $this->getNome() . ' azzanna ' . $this->target->getNome() . '!' . "\n";
		...
		public function s_1(){
			...
			$msg = '';
			if($this->getDistanceFrom($this->target) > 1) return false;
			$msg .= $this->getNome() . ' lacera ' . $this->target->getNome() . '!' . "\n";
			//$this->target->sendMessage($msg);
			$lf = $this->getTotalStat('FORZA');
